...
const ProfileDropdown = () => {
  const product = useSelector((state) => state?.products?.cartItem)
  console.log(product)
  ...
  return (
    <div className='flex gap-2 items-center justify-center'>
      <Link to='/cart' className='relative'>
        ...
        <p className="absolute top-[-5px] left-[10px] bg-red-500 text-white text-xs px-1 rounded-full">{product?.length | 0}</p>
      </Link>
      <Link to='/wishlist' className='relative'>
        <i className="text-2xl text-red-500 fa-solid fa-heart"></i>
        <p className="absolute top-[-5px] left-[10px] bg-red-500 text-white text-xs px-1 rounded-full">0</p>
      </Link>
